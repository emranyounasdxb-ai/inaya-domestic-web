...
function InstagramIcon() {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className={iconClass}>
      <defs>
        <linearGradient id="floatingInstagramGradient" x1="3" y1="21" x2="21" y2="3" gradientUnits="userSpaceOnUse">
      ...
      <rect width="20" height="20" x="2" y="2" rx="6" fill="url(#floatingInstagramGradient)" />
...
function IconLink({ href, label, children }: { href: string; label: string; children: ReactNode }) {
  return (
    <a
      ...
      aria-label={label}
      title={label}
      className="group relative inline-flex h-9 w-9 items-center justify-center rounded-full transition hover:bg-black/[0.04]"
    >
...
export default function FloatingSocialBar() {
  ...
  const links = [
    { href: phoneHref, label: 'Call INAYA Domestic Workers', icon: <PhoneIcon />, tone: 'text-gray-800' },
    { href: whatsappHref, label: 'WhatsApp INAYA Domestic Workers', icon: <WhatsAppIcon /> },
    { href: emailHref, label: 'Email INAYA Domestic Workers', icon: <EmailIcon />, tone: 'text-gray-800' },
