...
const countries: Country[] = [
  ...
  { flag: '🇳🇵', name: { en: 'Nepal', ar: 'نيبال' }, text: { en: 'Profiles may be available from Nepal and are matched according to experience, schedule and household needs.', ar: 'قد تتوفر ملفات من نيبال ويتم مطابقتها حسب الخبرة والجدول واحتياج المنزل.' }, points: { en: ['Experience', 'Schedule', 'Duties'], ar: ['الخبرة', 'الجدول', 'المهام'] } },
  { flag: '🇲🇲', name: { en: 'Myanmar', ar: 'ميانمار' }, text: { en: 'Profiles may be available from Myanmar depending on current availability, documents and service requirements.', ar: 'قد تتوفر ملفات من ميانمار حسب التوفر الحالي والمستندات ومتطلبات الخدمة.' }, points: { en: ['Availability', 'Documents', 'Service fit'], ar: ['التوفر', 'المستندات', 'ملاءمة الخدمة'] } },
  ...
  { flag: '🇬🇭', name: { en: 'Ghana', ar: 'غانا' }, text: { en: 'Profiles may be available from Ghana and are matched after reviewing documents, duties and family suitability.', ar: 'قد تتوفر ملفات من غانا وتتم مطابقتها بعد مراجعة المستندات والمهام وملاءمة الأسرة.' }, points: { en: ['Documents', 'Duties', 'Suitability'], ar: ['المستندات', 'المهام', 'الملاءمة'] } },
  { flag: '🇺🇬', name: { en: 'Uganda', ar: 'أوغندا' }, text: { en: 'Profiles may be available from Uganda. Suitability is reviewed through duties, documents and availability.', ar: 'قد تتوفر ملفات من أوغندا. تتم مراجعة الملاءمة حسب المهام والمستندات والتوفر.' }, points: { en: ['Duties', 'Documents', 'Availability'], ar: ['المهام', 'المستندات', 'التوفر'] } },
  ...
  { flag: '🇮🇩', name: { en: 'Indonesia', ar: 'إندونيسيا' }, text: { en: 'Profiles may be available from Indonesia depending on current availability and service requirements.', ar: 'قد تتوفر ملفات من إندونيسيا حسب التوفر الحالي ومتطلبات الخدمة.' }, points: { en: ['Service scope', 'Availability', 'Home routine'], ar: ['نطاق الخدمة', 'التوفر', 'روتين المنزل'] } }
];
...
export default function CountrySourcePage({ locale }: { locale: string }) {
  ...
  return (
    <main className="overflow-hidden bg-[#fbfaf7] text-primary-900">
      ...
      <section className="px-6 py-10 lg:px-10"><div className="mx-auto max-w-6xl"><h2 className="text-center font-heading text-3xl font-bold tracking-[-0.04em]">{t.cardsTitle}</h2><div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{countries.map((country) => <article key={country.name.en} className="rounded-[24px] border border-white/80 bg-white/80 p-5 shadow-[0_18px_48px_rgba(7,22,74,0.06)]"><div className="flex items-center gap-3"><span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#f5efe1] text-3xl shadow-inner">{country.flag}</span><h3 className="font-heading text-xl font-bold">{country.name[lang]}</h3></div><p className="mt-4 text-sm leading-6 text-primary-900/70">{country.text[lang]}</p><div className="mt-4 flex flex-wrap gap-2">{country.points[lang].map((point) => <span key={point} className="rounded-full border border-accent-500/20 bg-accent-50/60 px-3 py-1 text-[0.68rem] font-bold text-primary-900/75">{point}</span>)}</div></article>)}</div></div></section>
