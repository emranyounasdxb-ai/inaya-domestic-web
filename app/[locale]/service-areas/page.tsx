...
function CoverageVisual({ isArabic }: { isArabic: boolean }) {
  ...
  return (
    <div className="glass-panel relative min-h-[280px] overflow-hidden rounded-[24px] sm:min-h-[340px]">
      ...
      <div className="absolute inset-0 [background-image:radial-gradient(rgba(191,164,106,0.22)_1px,transparent_1px)] [background-size:28px_28px]" />
      <div className={`absolute top-5 z-10 flex max-w-[78%] items-center gap-3 rounded-full border border-accent-500/45 bg-white/72 py-2 pl-2 pr-5 text-primary-900 shadow-glass backdrop-blur-xl ${isArabic ? 'right-5 flex-row-reverse pl-5 pr-2 text-right' : 'left-5'}`}>
        ...
        <span className="min-w-0"><span className="block truncate text-sm font-bold sm:text-base">{title}</span><span className="block truncate text-xs font-semibold text-primary-900/75">{subtitle}</span></span>
      </div>
...
export default function ServiceAreasPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  return (
    <div className="overflow-hidden bg-ivory text-ink">
      ...
      <section className="container-x py-10 sm:py-12"><div className="mx-auto grid max-w-6xl grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">{stats.map((stat) => <div key={stat.label} className="glass-panel rounded-[18px] p-4 text-center sm:p-6"><div className={`${isArabic ? 'font-arabic text-2xl leading-snug' : 'font-heading text-3xl'} font-bold text-primary-900`}>{stat.value}</div><div className="mt-2 text-[0.65rem] font-bold uppercase tracking-[0.12em] text-primary-900/75 sm:text-xs">{stat.label}</div></div>)}</div></section>
