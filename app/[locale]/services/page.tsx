...
export default function ServicesPage({ params: { locale } }: { params: { locale: string } }) {
  ...
  const copy = {
    ...
    standardText: isArabic ? 'كل خدمة من عناية مدعومة بدعم شامل، مما يضمن راحة البال، الثقة، وتجربة منزلية منظمة.' : 'Every INAYA placement is backed by our comprehensive support ecosystem, ensuring seamless life, built long-term household value.',
    ...
    addSub: isArabic ? 'عزز باقة الدعم المنزلي بخدمات تشغيلية متخصصة.' : 'Enhance your core staffing package with these specialized, on-demand services to cover every aspect of home management.',
    ...
    bespokeText: isArabic ? 'إلى جانب الخدمات الفردية، تصمم عناية باقات دعم منزلي متكاملة للأسر، تشمل تنسيق عدة أفراد وخدمات بسلاسة.' : 'Beyond individual services, INAYA curates comprehensive, tailored household management packages. We assess your unique lifestyle requirements to design a holistic support structure, coordinating multiple staff members flawlessly.',
  ...
  const admin = [
    { icon: 'briefcase' as IconName, slug: 'recruitment', title: { en: 'Domestic Worker Recruitment', ar: 'توظيف العمالة المنزلية' }, text: { en: 'End-to-end recruitment services, sourcing top-tier domestic talent globally to meet your exacting standards.', ar: 'خدمة توظيف متكاملة لاختيار العمالة المنزلية المناسبة وفق احتياج الأسرة.' } },
    { icon: 'file' as IconName, slug: 'maid-visa', title: { en: 'Visa Assistance & Sponsorship', ar: 'المساعدة في التأشيرة والكفالة' }, text: { en: 'Seamless handling of all legalities, visa processing, and sponsorship transfers in full compliance with UAE law.', ar: 'متابعة إجراءات التأشيرة والكفالة والمستندات وفق متطلبات الإمارات.' } },
    { icon: 'shield' as IconName, slug: 'maid-replacement', title: { en: 'Replacement & Verification', ar: 'الاستبدال والتحقق' }, text: { en: 'Comprehensive background checks and guaranteed replacement services for total peace of mind.', ar: 'تحقق من الخلفية وخدمات استبدال منظمة لراحة بالك.' } }
  ];

  const bullets = isArabic ? ['دعم شامل', 'مدير حساب مخصص', 'تدريب مستمر'] : ['Comprehensive Insurance', 'Dedicated Account Manager', 'Continuous Training'];
      ...
      <section className="relative bg-[linear-gradient(120deg,#EEF6FF_0%,#F8F6F0_58%,#F6EFD9_100%)] px-6 py-20 text-primary-900 sm:py-24 lg:px-10 lg:py-28">
        <div className="mx-auto max-w-5xl text-center">
          ...
          <div className="mx-auto mt-10 grid max-w-3xl gap-5 sm:grid-cols-3">
            {stats.map((item) => <div key={item.value} className="rounded-[14px] bg-white/80 px-6 py-5 shadow-[0_20px_55px_rgba(7,22,74,0.08)]"><div className="font-heading text-4xl font-bold text-primary-900">{item.value}</div><div className="mt-2 text-[0.62rem] font-bold uppercase tracking-[0.18em] text-primary-900/55">{item.label}</div></div>)}
          </div>
        </div>
      </section>

      <section className="bg-black px-6 py-20 sm:py-24 lg:px-10 lg:py-28">
        <div className="mx-auto max-w-6xl">
          <div className="mx-auto max-w-3xl text-center"><h2 className={`${isArabic ? 'font-arabic' : 'font-heading'} text-[2.2rem] font-bold tracking-[-0.04em] text-primary-900 sm:text-[3rem]`}>{copy.coreTitle}</h2><p className="mt-4 text-sm leading-7 text-white/48">{copy.coreSub}</p></div>
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {core.map((item) => <Link key={item.slug} href={`/${locale}/services/${item.slug}`} className="group rounded-[14px] bg-white p-7 text-ink shadow-[0_22px_70px_rgba(0,0,0,0.25)] transition hover:-translate-y-1"><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-[#f3f1f3] text-primary-900"><LineIcon name={item.icon} className="h-5 w-5" /></div><p className="mt-8 text-[0.65rem] font-bold uppercase tracking-[0.18em] text-accent-700">{item.label[lang]}</p><h3 className={`${isArabic ? 'font-arabic' : 'font-heading'} mt-2 text-[1.42rem] font-bold leading-tight tracking-[-0.035em] text-primary-900`}>{item.title[lang]}</h3><p className="mt-4 min-h-[7.2rem] text-sm leading-7 text-ink/62">{item.text[lang]}</p><span className="mt-5 inline-flex items-center gap-2 text-xs font-bold text-primary-900">{item.cta[lang]} <LineIcon name="arrow" className="h-3.5 w-3.5 transition group-hover:translate-x-1 rtl:group-hover:-translate-x-1" /></span></Link>)}
          </div>
        </div>
      </section>

      <section className="bg-[#f5f2f4] px-6 py-20 text-primary-900 sm:py-24 lg:px-10 lg:py-28">
        <div className="mx-auto max-w-6xl"><div className="mx-auto max-w-3xl text-center"><h2 className={`${isArabic ? 'font-arabic' : 'font-heading'} text-[2.1rem] font-bold tracking-[-0.04em] sm:text-[3rem]`}>{copy.adminTitle}</h2><p className="mt-4 text-sm leading-7 text-primary-900/54">{copy.adminSub}</p></div><div className="mt-12 grid gap-6 md:grid-cols-3">{admin.map((item) => <Link key={item.slug} href={`/${locale}/services/${item.slug}`} className="rounded-[14px] border border-primary-900/6 bg-white/78 p-7 shadow-[0_18px_55px_rgba(7,22,74,0.05)] transition hover:-translate-y-1 hover:border-accent-500/35"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#f3f1f3] text-primary-900"><LineIcon name={item.icon} className="h-5 w-5" /></div><h3 className={`${isArabic ? 'font-arabic' : 'font-heading'} mt-7 text-[1.3rem] font-bold leading-tight tracking-[-0.03em]`}>{item.title[lang]}</h3><p className="mt-4 min-h-[6rem] text-sm leading-7 text-primary-900/58">{item.text[lang]}</p><span className="mt-5 inline-flex items-center gap-2 text-xs font-bold text-primary-900">{isArabic ? 'اعرف المزيد' : 'Learn More'} <LineIcon name="arrow" className="h-3.5 w-3.5" /></span></Link>)}</div></div>
      </section>

      <section className="bg-black px-6 py-20 sm:py-24 lg:px-10 lg:py-28">
        <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[1fr_0.9fr] lg:items-center">
          <div><h2 className={`${isArabic ? 'font-arabic' : 'font-heading'} text-[2rem] font-bold tracking-[-0.04em] text-primary-900 sm:text-[2.7rem]`}>{copy.standardTitle}</h2><p className="mt-5 max-w-xl text-sm leading-7 text-white/58">{copy.standardText}</p><div className="mt-9 space-y-6">{bullets.map((item) => <div key={item} className="flex gap-4"><span className="mt-1 h-8 w-8 shrink-0 rounded-full bg-accent-500" /><div><h3 className={`${isArabic ? 'font-arabic' : 'font-heading'} text-lg font-bold text-accent-300`}>{item}</h3><p className="mt-1 text-sm leading-6 text-white/58">{isArabic ? 'تفاصيل واضحة ودعم منظم لكل أسرة.' : 'Full medical and liability coverage included standard with all permanent placements.'}</p></div></div>)}</div></div>
          <div className="rounded-[18px] bg-[#f8f6f0] p-7 text-primary-900 shadow-[0_25px_70px_rgba(0,0,0,0.28)]"><h3 className={`${isArabic ? 'font-arabic' : 'font-heading'} text-2xl font-bold`}>{copy.compareTitle}</h3><div className="mt-7 space-y-3"><div className="grid grid-cols-2 rounded-lg bg-white px-4 py-3 text-sm"><span className="font-bold">{tiers[0]}</span><span className="text-end text-primary-900/60">{tiers[1]}</span></div><div className="grid grid-cols-2 rounded-lg bg-white px-4 py-3 text-sm"><span className="font-bold">{tiers[2]}</span><span className="text-end text-primary-900/60">{tiers[3]}</span></div><div className="grid grid-cols-2 rounded-lg bg-primary-900 px-4 py-3 text-sm text-white"><span className="font-bold">{tiers[4]}</span><span className="text-end text-accent-300">{tiers[5]}</span></div></div></div>
        </div>
      </section>

      <section className="bg-primary-900 px-6 py-20 text-white sm:py-24 lg:px-10 lg:py-28">
        <div className="mx-auto max-w-6xl"><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><h2 className={`${isArabic ? 'font-arabic' : 'font-heading'} max-w-xl text-[2rem] font-bold leading-tight tracking-[-0.04em] sm:text-[2.75rem]`}>{copy.addTitle}</h2><p className="mt-4 max-w-2xl text-sm leading-7 text-white/58">{copy.addSub}</p></div><Link href={`/${locale}/contact`} className="inline-flex w-fit items-center justify-center rounded-full border border-accent-300/35 px-5 py-2.5 text-xs font-bold text-white hover:bg-white/10">{copy.allServices}</Link></div><div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{addOns.map((item) => <div key={item.title.en} className="rounded-[12px] border border-white/10 bg-white/6 p-6"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-accent-500/12 text-accent-300"><LineIcon name={item.icon} className="h-4.5 w-4.5" /></div><h3 className={`${isArabic ? 'font-arabic' : 'font-heading'} mt-6 text-lg font-bold`}>{item.title[lang]}</h3><p className="mt-3 text-sm leading-6 text-white/58">{item.text[lang]}</p></div>)}</div></div>
      </section>

      <section className="bg-black px-6 py-20 sm:py-24 lg:px-10 lg:py-28">
        <div className="mx-auto max-w-6xl rounded-[18px] bg-[#f8f6f0] p-8 text-primary-900 shadow-[0_30px_90px_rgba(0,0,0,0.35)] sm:p-12"><div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-[0.66rem] font-bold uppercase tracking-[0.24em] text-accent-700">{copy.bespokeLabel}</p><h2 className={`${isArabic ? 'font-arabic' : 'font-heading'} mt-4 max-w-xl text-[2rem] font-bold leading-tight tracking-[-0.04em] sm:text-[2.8rem]`}>{copy.bespokeTitle}</h2><p className="mt-5 max-w-3xl text-sm leading-7 text-primary-900/62">{copy.bespokeText}</p></div><Link href={`/${locale}/contact`} className="inline-flex items-center justify-center rounded-full bg-primary-900 px-7 py-3 text-sm font-bold text-white shadow-[0_18px_45px_rgba(7,22,74,0.2)] transition hover:-translate-y-0.5">{copy.consult} <LineIcon name="arrow" className="ms-2 h-4 w-4" /></Link></div></div>
      </section>
    </div>
  );
}
